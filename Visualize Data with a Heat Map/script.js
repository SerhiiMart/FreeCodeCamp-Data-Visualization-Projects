...
fetch('https://raw.githubusercontent.com/freeCodeCamp/ProjectReferenceData/master/global-temperature.json').then(e => e.json()).then(e => {
  ...
  workOnData(monthlyVariance.map(e => ({...e, temp: baseTemperature - e.variance})));
})

function workOnData(data) {
  const graphHeight = (height - (2*padding)) / 12;
  const graphWidth = width / Math.floor(data.length / 12);
  ...
  svg.selectAll('rect')
    ...
    .attr('y', d => yScale(d.month - 1) - graphHeight)
  ...
  svg.append('g')
    .attr('id', 'y-axis')
    .attr('transform', `translate(${padding}, ${-graphHeight})`)
    .call(yAxis);
